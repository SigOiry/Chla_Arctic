...
Cropping_CCI <- function(path, 
  ...
  for (i in 1:nrow(list_img)) {
    ...
    if (file.exists(paste0("Data/CCI_v5/Cropped_TIF/",list_img$names[i])%>%gsub(".nc",".tif",.)) == F) {
      ...
      dir.create("Data/CCI_v5/Cropped_TIF/", showWarnings = F,recursive = T)
      ...
      if (saveNC) {
        ...
        dir.create("Data/CCI_v5/Cropped_NC/", showWarnings = F,recursive = T)
        writeCDF(img,
...
GSM <- function(path_cropped, lower_bounds, upper_bounds, lambda){
  ...
  for (date_i in 1:length(unique(list_img$date))) {
    ...
    output_name <- paste0(unique(list_img$date)[date_i],"_CHL_GSM-4km.nc")
    ...
    if (file.exists(paste0("Data/GSM/CCI_v5/",output_name)) == F ) {
      ...
      dir.create("Data/GSM/CCI_v5/", showWarnings = F,recursive = T)
...
#  - saveNC : TRUE or FALSE, if TRUE NC will be saved at the cropping step. default = F
