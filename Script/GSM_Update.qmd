...
Cropping_CCI <- function(list_img, xmin = -180 ,xmax = 180 ,ymin = 64 , ymax = 90, saveNC = F){
...
GSM <- function(list_img, lower_bounds = c(0, 0.0001, 0.0001), upper_bounds = c(64, 2, 0.1), lambda =  c(412, 443, 490, 510, 560, 665), img_bw_NoCoords = NA){
...
Batch_processing <- function(path = "Data/CCI_v5/NC", lower_bounds = c(0, 0.0001, 0.0001), upper_bounds = c(64, 2, 0.1), lambda =  c(412, 443, 490, 510, 560, 665), xmin = -180 ,xmax = 180 ,ymin = 64 , ymax = 90, saveNC = F){
  ...
  Cropping_CCI(list_img)
  
  list_img <- list_img %>% 
    mutate(path = gsub("NC","Cropped_TIF",path) %>% 
             gsub(".nc",".tif",.),
           names = gsub(".nc",".tif",names))
...
Batch_processing("Data/CCI_v5/NC")
